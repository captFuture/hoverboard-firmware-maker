...
void dumpGamepad(ControllerPtr ctl) {
  ...
  if(JoyAxisY > ThreshHold && JoyAxisY < MinusThreshHold){
    JoyAxisY == 0;
  }
  if(JoyAxisRY > ThreshHold && JoyAxisRY < MinusThreshHold){
    JoyAxisY == 0;
  }
...
void processGamepad(ControllerPtr ctl) {
  ...
  if (ctl->thumbR()) {
    Serial.printf("reversing Motors");
    rewDir = !rewDir;
  }
  if (ctl->r1()) {
  ...
  if (ctl->dpad()) {
    ...
    if(ctl->dpad() == 8){
      ThreshHold = ThreshHold - 10; if(ThreshHold < 0){ThreshHold = 0;}
      MinusThreshHold = MinusThreshHold + 10; if(MinusThreshHold > 0){MinusThreshHold = 0;}
    }else if(ctl->dpad() == 4){
      ThreshHold = ThreshHold + 10; if(ThreshHold > 100){ThreshHold = 1000;}
      MinusThreshHold = MinusThreshHold - 10; if(MinusThreshHold < -100){MinusThreshHold = -100;}
    }
  }
...
void setup() {
  ...
  odrive_serial.begin(115200, SERIAL_8N1, GPIO_PIN36, GPIO_PIN26);
  Serial.begin(115200);
