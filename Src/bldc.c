...
/*
// from https://github.com/EFeru/hoverboard-firmware-hack-FOC/issues/185
boolean_T bAngleInit = true;
uint16_t AngleLeft = 0;// cyclic angle variables scaled in motor elec degrees
uint16_t AngleRight = 0;
static uint16_t AngleLeftLast = 0;
static uint16_t AngleRightLast = 0;
static int cycleDegsL = 0 ; //wheel rotation roll over count in elec deg
static int cycleDegsR = 0 ; //wheel rotation roll over count in elec deg
int16_t diffL = 0;
int16_t diffR = 0;
static int cnt = 0;
*/
...
uint8_t        enable2       = 1;        //robo global variable for additional ENABLE_TIMEOUT 
...
void DMA1_Channel1_IRQHandler(void) {
  ...
  if(ABS(curL_DC) > curDC_max || enable == 0 || enable2 == 0) {
    LEFT_TIM->BDTR &= ~TIM_BDTR_MOE;
  } else {
    LEFT_TIM->BDTR |= TIM_BDTR_MOE;
  }

  if(ABS(curR_DC)  > curDC_max || enable == 0 || enable2 == 0) {
  ...
  enableFin = enable && enable2 && !rtY_Left.z_errCode && !rtY_Right.z_errCode;		// robo
...
//	AngleLeft = rtY_Left.a_elecAngle;	
 ...
 // motAngleRight = rtY_Right.a_elecAngle;
    ...
    // robo22
	//AngleRight = rtY_Right.a_elecAngle;	
	encoding = (uint8_t)((hall_ur<<2) + (hall_vr<<1) + hall_wr);
    iWheelPos = rtConstP.vec_hallToPos_Value[encoding];
    //iOdomR = modulo(iOdomR - up_or_down(iWheelPosLastR, iWheelPos), 9000);
